...
beforeAll(async () => {
  ...
  const client = await Client.create({
    ...
    name: 'ClientA',
    cif: 'A1',
    address: { street: 'X', number: 1, postal: 11111, city: 'City', province: 'Prov' },
  });
  clientId = client._id;
...
describe('Projects controller (routes under /api/projects)', () => {
  // GET /api/projects
  ...
  describe('POST /api/projects', () => {
    ...
    test('500 – save error', async () => {
      jest.spyOn(Project.prototype, 'save').mockRejectedValueOnce(new Error('fail'));
      const res = await request(app)
        .post('/api/projects')
        .set('Authorization', `Bearer ${token}`)
  ...
  describe('PUT /api/projects/:id', () => {
    ...
    test('500 – DB failure', async () => {
      jest.spyOn(Project, 'findOneAndUpdate').mockRejectedValueOnce(new Error('fail'));
      const res = await request(app)
        .put(`/api/projects/${projectId}`)
        .set('Authorization', `Bearer ${token}`)
        .send({ name: 'Err' });
  ...
  describe('DELETE /api/projects/:id', () => {
    test('200 – hard deletes project', async () => {
      const tmp = await Project.create({ userId, clientId, name: 'TmpProj', archived: false });
      const res = await request(app)
    ...
    test('500 – DB failure', async () => {
      jest.spyOn(Project, 'findOneAndDelete').mockRejectedValueOnce(new Error('fail'));
      const res = await request(app)
  ...
  // PATCH /api/projects/archive/:id & /api/projects/restore/:id
